...
std::map<graphlab::vertex_id_type, float> *g_degrees = new std::map<graphlab::vertex_id_type, float>();

bool node_has_degree(graphlab::vertex_id_type i){
	return g_degrees->find(i) != g_degrees->end();
}
...
bool line_parser(graph_type& graph, const std::string& filename, const std::string& textline) {
  ...
  (*g_degrees)[vid2]+=weight;
  if (graph_is_directed) (*g_degrees)[vid1]+=weight;
  ...
  public:
    ...
    void apply(icontext_type& context, vertex_type& vertex, const gather_type& total) {
      ...
      for ( std::map<vertex_data_type, edge_data_type>::const_iterator iter = total.label_count.begin();
		...
		if ((*g_degrees)[maxLabel]<(*g_degrees)[iter->first])       maxLabel = iter->first;
	      }
      }
